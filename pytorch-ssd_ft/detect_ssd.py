...
# Save the currently processed frame id and sleep for heartbeat interval
def heartbeat_fn():
...
#if Fault tolerant flag is enabled, start fault tolerant process
if ft_flag :
    clean_up_files()
    delete_checkpoints()
    heart_bt_thread = threading.Thread(target=heartbeat_fn)
    heart_bt_thread.start()
...
for image_path in TEST_IMAGE_PATHS:
    ...
    if frm_cnt > 15:  # ignore first a few frame result
        ...
        if interval > max_exe:
            max_exe = interval
        print('FPS: {:.2f}'.format(frame_fps) + " ,  Frame ID : " + str(curr_frame_id+1))
    ...
    #            break;
    #        cv2.waitKey(1);
    #Update the currently processed frame id
    if ft_flag:
        frame_id_mutex.acquire()
        curr_frame_id += 1
        frame_id_mutex.release()
